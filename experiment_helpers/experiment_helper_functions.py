...
def add_salt_and_pepper(image, prob=0.12):
    noisy = np.copy(image)
...
def add_motion_blur(image, kernel_size=21):
    kernel = np.zeros((kernel_size, kernel_size))
    kernel[:, kernel_size // 2] = 1.0 / kernel_size
    return cv2.filter2D(image, -1, kernel)


def add_glass_blur(image, kernel_size=15, iterations=2):
    """Applies glass blur (also known as diffusion blur) to an image."""
    h, w, c = image.shape
    image = image.astype(np.float32) / 255.0  # Normalize image

    # Create random displacement field
    displacement = np.random.randint(-kernel_size, kernel_size + 1, size=(h, w, 2))
    ...
    for _ in range(iterations):
        ...
        displaced_x = np.clip(x + displacement[..., 0], 0, w - 1)
        displaced_y = np.clip(y + displacement[..., 1], 0, h - 1)

        # Reshape coordinates for map_coordinates
        indices = np.array([displaced_y.ravel(), displaced_x.ravel()])

        # Apply transformation to each channel
        for i in range(c):
            image[..., i] = map_coordinates(image[..., i], indices, order=1, mode='reflect').reshape(h, w)

    return (image * 255).astype(np.uint8)  # Convert back to uint8


def add_fog(image, fog_strength=.95):
    fog = np.full(image.shape, 255, dtype=np.uint8)
    return cv2.addWeighted(image, 1 - fog_strength, fog, fog_strength, 0)


def add_heat_distortion(image, strength=.1):
    h, w, _ = image.shape
    distortion = np.random.normal(scale=strength, size=(h, w))
    flow = np.stack([distortion, np.zeros_like(distortion)], axis=-1)
    return cv2.remap(image, (np.arange(w) + flow[..., 0]).astype(np.float32),
                     (np.arange(h) + flow[..., 1]).astype(np.float32), interpolation=cv2.INTER_LINEAR)


def add_chromatic_aberration(image, shift=10):
...
def add_radial_blur(image, strength=0.8):
    """Applies radial blur by blending the original image with a Gaussian-blurred version."""
    h, w, c = image.shape

    # Create a distance-based mask (center-focused)
    y, x = np.meshgrid(np.linspace(-1, 1, w), np.linspace(-1, 1, h))
    mask = np.sqrt(x ** 2 + y ** 2)
    mask = (mask - mask.min()) / (mask.max() - mask.min())  # Normalize mask to range [0,1]

    # Apply Gaussian blur
    blurred = cv2.GaussianBlur(image, (21, 21), 10)

    # Ensure mask is 3-channel to match the image
    mask = np.dstack([mask] * 3)

    # Blend images
    radial_blurred = (image * (1 - strength * mask) + blurred * (strength * mask)).astype(np.uint8)

    return radial_blurred


# def add_poisson_noise(image):
#     vals = 2 ** np.ceil(np.log2(np.max(image)))
#     noisy = np.random.poisson(image * vals) / vals
#     return np.clip(noisy, 0, 255).astype(np.uint8)

def add_rain(image, streak_length=15, density=0.02):
    rain_mask = np.zeros_like(image, dtype=np.uint8)
    num_streaks = int(density * image.size)

    for _ in range(num_streaks):
        x, y = np.random.randint(0, image.shape[1]), np.random.randint(0, image.shape[0])
        for i in range(streak_length):
            if y + i < image.shape[0]:
                rain_mask[y + i, x] = 255

    return cv2.addWeighted(image, 1, rain_mask, 0.2, 0)


def add_jpeg_artifacts(image, quality=3):
...
def apply_occlusion(image, occlusion_size=0.6, mode="white"):
    """
    Applies an occlusion to the image by placing a black, white, or blurred patch.

    Parameters:
        image (numpy.ndarray): Input image (H, W, C).
        occlusion_size (float): Size of occlusion as a fraction of image size (default 0.3).
        mode (str): Type of occlusion - "black", "white", or "blurred".

    Returns:
        numpy.ndarray: Image with occlusion applied.
    """
    h, w, c = image.shape
    patch_size = int(min(h, w) * occlusion_size)  # Determine occlusion size

    # Randomly select top-left corner of occlusion patch
    x1 = random.randint(0, w - patch_size)
    y1 = random.randint(0, h - patch_size)
    x2, y2 = x1 + patch_size, y1 + patch_size  # Bottom-right corner

    occluded_image = image.copy()

    if mode == "black":
        occluded_image[y1:y2, x1:x2] = 0  # Black patch
    elif mode == "white":
        occluded_image[y1:y2, x1:x2] = 255  # White patch
    # elif mode == "blurred":
    #     blurred_patch = cv2.GaussianBlur(image[y1:y2, x1:x2], (21, 21), 0)
    #     occluded_image[y1:y2, x1:x2] = blurred_patch  # Replace patch with blurred region

    return occluded_image


def apply_banding(image, frequency=20, intensity=100):
    """
    Adds a banding effect (horizontal sinusoidal pattern) to an image.

    Parameters:
        image (numpy.ndarray): Input image (H, W, 3).
        frequency (int): Controls the number of bands (higher = more bands).
        intensity (int): Strength of the effect.

    Returns:
        numpy.ndarray: Image with banding applied.
    """
    h, w, c = image.shape  # Get image dimensions

    # Generate a sinusoidal wave pattern across the height of the image
    banding = np.sin(np.linspace(0, np.pi * frequency, h))  # Shape: (H,)

    # Reshape it to match the full image width and all color channels
    banding = np.tile(banding[:, np.newaxis], (1, w))  # Shape: (H, W)
    banding = np.repeat(banding[:, :, np.newaxis], c, axis=2)  # Shape: (H, W, 3)

    # Apply banding effect to the image
    banded_image = np.clip(image.astype(np.float32) + (banding * intensity), 0, 255).astype(np.uint8)
...
def apply_wavy_distortion(image, frequency=20, amplitude=15):
    """
    Applies a wavy distortion effect to an image.
    ...
    Returns:
        numpy.ndarray: Image with wavy distortion applied.
    """
    h, w, c = image.shape  # Get image dimensions

    # Generate a sinusoidal wave pattern for vertical displacement
    wave = amplitude * np.sin(np.linspace(0, np.pi * frequency, h))  # Shape: (H,)

    # Reshape to match image dimensions
    wave = np.tile(wave[:, np.newaxis], (1, w))  # Shape: (H, W)
    wave = np.repeat(wave[:, :, np.newaxis], c, axis=2)  # Shape: (H, W, 3)

    # Apply distortion by shifting pixels
    distorted_image = np.zeros_like(image)
    for i in range(h):
        shift = int(wave[i, 0, 0])  # Extract shift value for the row
        distorted_image[i] = np.roll(image[i], shift, axis=0)  # Shift pixels horizontally

    return distorted_image


def add_gaussian_blur(image, kernel_size=21, sigma=5):
...
def local_imp_concepts_raw_soft(craft_instance, importances, num):
    # This gets three most important local concepts and adds up there importance globally with respect to each class and then finds the max
    image_preds = []
    # image_raw =[]
    for i, image_imp in enumerate(importances):
...
def global_imp_concepts_raw_hard(craft_instance, importances, num):
    # This method takes the top 3 global concepts from each class and sums their local importance. This method gets a 75% accuracy
    image_preds = []
    # image_raw =[]
    for i, image_imp in enumerate(importances):
...
def global_imp_concepts_raw_soft(craft_instance, importances, num):
    # This method takes the top 3 global concepts from each class and sums their local importance. This method gets a 75% accuracy
    image_preds = []
    # image_raw =[]
    for i, image_imp in enumerate(importances):
...
def accuracy_rejection(predictions, labels, uncertainty, plot=True, plot_random=False,
